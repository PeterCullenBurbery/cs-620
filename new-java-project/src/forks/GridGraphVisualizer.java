...
//		this.gridGraphString = buildGridGraphString();
		...
		System.out.println("Adjacency Matrix after DFS and edge removal:");
...
//        classifyForks();
	...
	public void printNonZeroValuesInAdjacencyMatrix() {
	    ...
	    for (int row = 0; row < adjacencyMatrix.length; row++) {
	        ...
	        // Count the nonzero values in the row
	        ...
	        // Print the row number (1-indexed) and the count of nonzero values
	        System.out.println((row + 1) + " " + nonZeroCount);
	    }
	}
	...
	// Method to classify correct and incorrect forks
	// Method to classify correct and incorrect forks
	// Method to classify correct and incorrect forks
	// Method to classify correct and incorrect forks
	// Method to classify correct and incorrect forks
//	private void classifyForks() {
//	    correctForks.clear();
//	    incorrectForks.clear();
//
//	    // Loop through each path fork node
//	    for (Node forkNode : pathForks) {
//	        List<Node> adjacentNodes = getAdjacentMazeNodes(forkNode);  // Get adjacent nodes in the maze
//	        boolean isIncorrect = false;
//	        System.out.println("Checking fork node: " + forkNode);
//	        System.out.println("Adjacent nodes for fork: " + adjacentNodes);
//	        System.out.println("Tried nodes: " + tried);
//	        // Check if any adjacent nodes are in the tried set (indicating an incorrect move)
//	        for (Node neighbor : adjacentNodes) {
//	            if (tried.contains(neighbor)) {
//	                isIncorrect = true;
//	                break;  // If even one adjacent node is in tried, classify as incorrect
//	            }
//	        }
//
//	        // Classify the fork based on whether it has adjacent tried nodes
//	        if (isIncorrect) {
...
//	    int nodeIndex = node.getNodeNumber(); // Assuming each node has a unique number for the matrix row/column.
//	    
//	    for (int i = 0; i < adjacencyMatrix[nodeIndex].length; i++) {
//	        if (adjacencyMatrix[nodeIndex][i] == 1 && i != nodeIndex) {  // Exclude the node itself
//	            Node adjacentNode = getNodeByIndex(i);  // Retrieve the node by its index
...
//	}
	...
	public int[] getAdjacencyRow(Node node) {
	    int nodeNumber = node.getNodeNumber();  // Get the node's number
	    if (nodeNumber >= 0 && nodeNumber < adjacencyMatrix.length) {
	        return adjacencyMatrix[nodeNumber];  // Return the nodeNumber-th row of the matrix
	    } else {
	...
	// Method to get all incident edges for a node
	private List<Edge> getIncidentEdges(Node node) {
	    List<Edge> incidentEdges = new ArrayList<>();
	    int nodeIndex = node.getNodeNumber();

	    // Loop through the adjacency matrix to find connected nodes (edges)
	    for (int i = 0; i < adjacencyMatrix[nodeIndex].length; i++) {
	        if (adjacencyMatrix[nodeIndex][i] == 1) {  // If there's an edge (1 in adjacency matrix)
	            incidentEdges.add(new Edge(node, nodes.get(i)));  // Create an edge and add it to the list
	        }
	    }
...
//			System.out.println(Arrays.toString(coord));
//		}
		GridGraphVisualizer gridGraph3x3 = new GridGraphVisualizer(7,7/*3,3*/);
...
//        
	...
	public void printNonZeroValuesForPathForks() {
	    ...
	    for (Node fork : pathForks) {
	        int nodeNumber = fork.getNodeNumber() - 1;  // Adjust for 0-based indexing
	        int nonZeroCount = 0;
	        ...
	        if (nodeNumber >= 0 && nodeNumber < adjacencyMatrix.length) {
	            ...
	            // Print the node number (1-indexed) and the count of nonzero values
	            System.out.println((nodeNumber + 1) + " " + nonZeroCount);
	        } else {
	            // Handle cases where nodeNumber is out of bounds
	            System.out.println("Invalid node number: " + (nodeNumber + 1));
	        }
	    }
	}
...
/*
 * find tried edges
 * get all the nodes from the edges by combining the start nodes and end nodes from tried edges into one
 * big list. Remove duplicates from this list of nodes. Count how many nodes from path are in this list.
 * Or to put it another way, take the intersection between this list and your path nodes, and find the
 * length /cardinality/number of elements in this intersection.
 * 
 * 
 * 
 * 
